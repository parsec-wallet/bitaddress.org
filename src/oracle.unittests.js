(function (parsec) {
	var ut = parsec.unitTests = {
		...
		runAsynchronousTests: function (showOutput) {
			...
			parsec.foreachSerialized(parsec.unitTests.asynchronousTests, function (name, cb) {
				//Bitcoin.KeyPool.reset();
				document.getElementById("busyblock").className = "busy";
				parsec.unitTests.asynchronousTests[name](cb);
			}, function () {
				if (showOutput) {
		...
		synchronousTests: {
			...
			testIsPublicKeyHexFormat: function () {
				...
				var bool = parsec.publicKey.isPublicKeyHexFormat(key);
				if (bool != true) {
					return false;
				}
				return true;
			...
			testGetHexFromByteArray: function () {
				...
				var key = parsec.publicKey.getHexFromByteArray(bytes);
			...
			testGetBitcoinAddressFromByteArray: function () {
				var bytes = [4, 120, 152, 47, 64, 250, 12, 11, 122, 85, 113, 117, 131, 175, 201, 154, 78, 223, 211, 1, 162, 114, 157, 197, 155, 11, 142, 185, 225, 134, 146, 188, 181, 33, 240, 84, 250, 217, 130, 175, 76, 193, 147, 58, 253, 31, 27, 86, 62, 167, 121, 166, 170, 108, 206, 54, 163, 11, 148, 125, 214, 83, 230, 62, 68];
				var address = parsec.publicKey.getBitcoinAddressFromByteArray(bytes);
			...
			testGetByteArrayFromAdding: function () {
				...
				var bytes = parsec.publicKey.getByteArrayFromAdding(key1, key2);
			...
			testGetByteArrayFromAddingCompressed: function () {
				...
				var bytes = parsec.publicKey.getByteArrayFromAdding(key1, key2);
				var hex = parsec.publicKey.getHexFromByteArray(bytes);
			...
			testGetByteArrayFromMultiplying: function () {
				...
				var bytes = parsec.publicKey.getByteArrayFromMultiplying(key1, new Bitcoin.ECKey(key2));
			...
			testGetByteArrayFromMultiplyingCompressedOutputsCompressed: function () {
				...
				var bytes = parsec.publicKey.getByteArrayFromMultiplying(key1, ecKey);
			...
			testGetPubHexFromMultiplyingPrivAPubB: function () {
				...
				var bytes = parsec.publicKey.getByteArrayFromMultiplying(keyPub, new Bitcoin.ECKey(keyPriv));
				var pubHex = parsec.publicKey.getHexFromByteArray(bytes);
			...
			testGetECKeyFromAdding: function () {
				...
				var key2 = "SQE6yipP5oW8RBaStWoB47xsRQ8pat";
				var ecKey = parsec.privateKey.getECKeyFromAdding(key1, key2);
			...
			testGetECKeyFromMultiplying: function () {
				var key1 = "5J8QhiQtAiozKwyk3GCycAscg1tNaYhNdiiLey8vaDK8Bzm4znb";
				var key2 = "SQE6yipP5oW8RBaStWoB47xsRQ8pat";
				var ecKey = parsec.privateKey.getECKeyFromMultiplying(key1, key2);
			...
			testDecodePointHexForCompressedPublicKey: function () {
				...
				var pubHexUncompressed = parsec.publicKey.getDecompressedPubKeyHex(key);
			...
			testSplitAndCombinePrivateKey2of2: function () {
				...
				var shares = parsec.wallets.splitwallet.getFormattedShares(key, numshares, threshold);
				var combined = parsec.wallets.splitwallet.combineFormattedShares(shares);
		...
		asynchronousTests: {
			...
			testBip38: function (done) {
				...
				var decryptBip38Test = function (test, i, onComplete) {
					parsec.privateKey.BIP38EncryptedKeyToByteArrayAsync(test[0], test[1], function (privBytes) {
				...
				var encryptBip38Test = function (test, compressed, i, onComplete) {
					parsec.privateKey.BIP38PrivateKeyToEncryptedKeyAsync(test[2], test[1], compressed, function (encryptedKey) {
				...
				var cycleBip38Test = function (i, compress, onComplete) {
					...
					// encrypt private key
					parsec.privateKey.BIP38PrivateKeyToEncryptedKeyAsync(privKey, 'testing', compress, function (encryptedKey) {
						// decrypt encryptedKey
						parsec.privateKey.BIP38EncryptedKeyToByteArrayAsync(encryptedKey, 'testing', function (decryptedBytes) {
				...
				var intermediateBip38Test = function (i, onComplete) {
					...
					parsec.privateKey.BIP38GenerateIntermediatePointAsync(pass, null, null, function (intermediatePoint) {
						parsec.privateKey.BIP38GenerateECAddressAsync(intermediatePoint, false, function (address, encryptedKey) {
							parsec.privateKey.BIP38EncryptedKeyToByteArrayAsync(encryptedKey, pass, function (privBytes) {
								if (privBytes.constructor == Error) {
				...
				var testArray = [
					function (cb) {
						...
						parsec.forSerialized(0, tests.length, function (i, callback) {
					...
					function (cb) {
						...
						parsec.forSerialized(0, 4, function (i, callback) {
					...
					function (cb) {
						...
						parsec.forSerialized(0, 2, function (i, callback) {
					...
					function (cb) {
						...
						parsec.forSerialized(0, 5, function (i, callback) {
				...
				parsec.runSerialized(testArray, done);
			}
		}
	};
})(parsec);